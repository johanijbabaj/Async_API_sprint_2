...
async def test_search_film(some_film, make_get_request):
    ...
    response = await make_get_request("/search", {"query_string": "Some"})
...
@pytest.mark.asyncio
async def test_search_empty(empty_film_index, make_get_request):
    """Тест запускается без фикстур и API должен вернуть ошибку 404"""
    response = await make_get_request("/search")
...
@pytest.mark.asyncio
async def test_no_index(make_get_request):
